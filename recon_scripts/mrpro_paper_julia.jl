...
csm   = read(fid["csm"])
matrix_size = read(fid["matrix_size"])
n_k0 = read(fid["n_k0"])
n_k1 = read(fid["n_k1"])
...
csm = reshape(ComplexF32.(csm), matrix_size, matrix_size, 1, n_coils)

tr = Trajectory(ktraj, n_k1, n_k0, circular=true)
...
result = @benchmark reconstruction($acqData, $params)
println(result)
